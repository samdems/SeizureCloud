...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('medication_reminder_logs', function (Blueprint $table) {
            ...
            $table->index(['user_id', 'medication_schedule_id', 'sent_at']);
            $table->index(['user_id', 'sent_at']);
            $table->index(['medication_schedule_id', 'sent_at']);
            $table->index(['sent_at']);

            // Composite index for spam prevention queries
            $table->index(['medication_schedule_id', 'reminder_type', 'sent_at'], 'med_reminder_spam_prevention');
        });
    }
