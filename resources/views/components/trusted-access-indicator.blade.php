@if(session('viewing_as_trusted_contact') && session('original_user_id'))
    @php
        $originalUser = \App\Models\User::find(session('original_user_id'));
        $trustedUser = auth()->user();
    @endphp

    <div {{ $attributes->merge(['class' => 'alert alert-warning']) }}>
        <div class="flex items-center gap-3">
            <div class="badge badge-warning">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
                Trusted Access
            </div>
            <div class="flex-1">
                <p class="font-medium">You are viewing {{ $trustedUser->name }}'s account as {{ $originalUser->name }}</p>
                @if(isset($showSwitchButton) && $showSwitchButton)
                    <form action="{{ route('trusted-access.switch-back') }}" method="POST" class="mt-2">
                        @csrf
                        <button type="submit" class="btn btn-xs btn-outline btn-warning">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" />
                            </svg>
                            Switch back to your account
                        </button>
                    </form>
                @endif
            </div>
        </div>
    </div>
@endif
